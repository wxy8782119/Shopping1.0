...
//		}
		...
		String oid = CommonsUtils.getUUID();// 该订单的订单号
		order.setOid(oid);
		...
		order.setState(0);// 订单支付状态
		order.setAddress(null);// 收货地址
		order.setName(null);// 收货人
	...
	public void addProductToCart(HttpServletRequest request, HttpServletResponse response)
		...
		// 将购物项放到车中---key是pid
		// 先判断购物车中是否已经包含此购物项了-----判断key是否可以存在
...
//		request.getRequestDispatcher("/cart.jsp").forward(request, response);
	...
	public void categoryList(HttpServletRequest request, HttpServletResponse response)
		...
		// 先从缓存中查询categoryList，如果有就直接使用，没有就在从数据库中查询存到缓存中
